...
ROOT_ALIASES: Dict[str, str] = {
    ...
    _norm_keep_numbers("манки"): "Monkey Shoulder Blended Malt",
    _norm_keep_numbers("манки шолдер"): "Monkey Shoulder Blended Malt",
...
def all_brand_names() -> List[str]:
    ...
    out = []
    for rec in data:
        nm = rec.get("brand") or rec.get("name")
        if nm and nm not in out:
            out.append(nm)
    return out


def suggest_candidates(query: str, top_n: int = 15):
    try:
        pairs = fuzzy_suggest(query, limit=top_n)
        return [p[0] if isinstance(p, tuple) else str(p) for p in pairs][:top_n]
    except Exception:
        return []
...
def brand_card(name: str) -> Optional[str]:
    ...
    if not rec:
        return None
    title = rec.get("brand") or rec.get("name") or nm
    bits = []
    meta = " | ".join([x for x in [rec.get("category"), rec.get("country"), rec.get("abv")] if x])
    if meta: bits.append(meta)
    tn = rec.get("tasting_notes")
    if tn:
        bits.append("Вкус: " + (", ".join(tn) if isinstance(tn, list) else str(tn)))
    facts = rec.get("production_facts")
    if facts:
        if isinstance(facts, list):
            for f in facts[:3]:
                bits.append("• " + str(f))
        else:
            bits.append(str(facts))
    serve = rec.get("serve")
    if serve:
        bits.append("Подача: " + (serve if isinstance(serve, str) else ", ".join(serve)))
