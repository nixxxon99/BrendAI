...
def load_names() -> Set[str]:
    ...
    if not all_rows:
        print('[portfolio] No CSV found in data/*.csv')
        _names_cache=set(); return _names_cache
    ci = _find_name_col(all_rows) or 0
    names = []
    src_files = [str(c) for c in CANDIDATES if c.exists()]
    header_passed = False
    for r in all_rows:
    ...
    print(f"[portfolio] Loaded {len(_names_cache)} names from: {', '.join(src_files) if src_files else 'data/*'}")
    return _names_cache
