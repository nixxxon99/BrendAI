...
def _load_portfolio() -> set[str]:
    ...
    try:
        ...
        pass
    # 2) try from files: json/txt/csv
    paths = [
        os.path.join("data", "portfolio.json"),
        os.path.join("data", "portfolio.txt"),
                ...
                with open(p, "r", encoding="utf-8") as f:
                    ...
                    for row in reader:
                        if not row:
                            continue
                        name = (row[0] or "").strip()
                        if not name or name.lower() in {"brand", "name", "бренд"}:
                            # вероятный заголовок
                            continue
                        _PORTFOLIO.add(name)
        except Exception:
            # не валим весь бот из-за плохого файла
            pass
    return _PORTFOLIO
...
def find_brand(query: str):
    ...
    al = _aliases()
    qlow = q.casefold()
    # 0) попытка вытащить бренд из фразы вида "как продавать X"
    extracted = _extract_brand_from_phrase(q)
    if extracted:
        qlow = extracted.casefold()
    # 1) alias → exact
    ...
    if qlow in idx:
        ...
        return idx[qlow], [nm]
    # 3) продвинутый suggest из services.brands
    try:
        ...
        names = []
    if names:
        # если явный лидер (разница по скору заложена в suggest — у нас нет, поэтому если 1 кандидат — берём)
        if len(names) == 1 and names[0].casefold() in idx:
            return idx[names[0].casefold()], [names[0]]
        # если среди кандидатов есть точные вхождения — берём первого
        for n in names:
            if n.casefold() in idx:
                return idx[n.casefold()], [n]
        return None, names
    # 4) простой подстрочный фолбэк
    hits = []
    ...
    if qlow in idx:
        nm = _brand_name(idx[qlow])
        return idx[qlow], [nm]
    # 3) use services.brands.suggest_candidates with transliteration
    try:
        from app.services.brands import suggest_candidates
        names = suggest_candidates(q, top_n=10)
    except Exception:
        names = []
    if names:
        # if single strong candidate – return as match
        if len(names) == 1 and names[0].casefold() in idx:
            return idx[names[0].casefold()], [names[0]]
        return None, names
    # 4) fallback substring over index
    hits = []
    for key, rec in idx.items():
        if qlow in key:
            hits.append(_brand_name(rec))
...
def answer_ai(query: str) -> str:
    ...
    if rec:
        ...
        cat = _brand_cat(rec)
        if not kb:
            if (cat or "").lower().startswith("whisk"):
                kb = _generic_sales_tips("виски")
            elif "конья" in (cat or "").lower():
                kb = _generic_sales_tips("коньяк")
            else:
                kb = _generic_sales_tips(cat or "")
        alts = []
        if status is False:
