...
def _ocr_available():
    try:
        from app.services.ocr import ocr_text, OCR_STATUS
        return True, getattr(OCR_STATUS, "last_error", None)
...
async def vision_enter(m: Message):
    ...
    if ok:
        await m.answer("Отправь фото (как фото, не документ) — попробую распознать и найти бренд.")
    else:
        await m.answer("OCR сейчас недоступен. Подключи Google Vision (см. README) или пришли более чёткое фото.\nОшибка: " + (err or "неизвестно"))
