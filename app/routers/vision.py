...
@router.message(F.text.in_({VISION_BUTTON_TEXT, 'Фото-анализ', '📸 Фото-анализ'}))
async def vision_enter(m: Message):
    await m.answer("Отправь фото как *фото* (не документ). Если OCR не подключён — подскажу, как включить.", parse_mode="Markdown")
