from app.keyboards.menus import AI_ENTRY_BUTTON_TEXT
...
async def ai_enter(m: Message):
    AI_USERS.add(m.from_user.id)
    await m.answer("Готов. Спроси: «как продавать <бренд>» или «что предложить взамен <бренд>».")

@router.message(lambda m: m.text is not None and m.from_user and m.from_user.id in AI_USERS)
async def ai_q(m: Message):
    # простейший режим: если пользователь нажал AI, отвечаем на любое текстовое сообщение
    ...
    # найдём предполагаемый бренд в запросе
    candidates = [name for name, score in (fuzzy_suggest(q, limit=1) or [])]
    name = candidates[0] if candidates else None

    if name and in_portfolio(name):
        # наш бренд — краткий скрипт продаж
        msg = (f"Это наш бренд: <b>{name}</b>\n"
               f"• Кому предлагать: гости, которым нравится стиль бренда из запроса.\n"
               f"• Как преподнести: 1-2 выгоды (вкус/стиль/страна).\n"
               f"• Альтернатива внутри портфеля: {', '.join(suggest_alternatives(name) or [])}")
        await m.answer(msg, parse_mode="HTML")
    else:
        # не наш — предложим альтернативы
        alts = suggest_alternatives(name or q) or []
        if alts:
            await m.answer("Это похоже на чужой бренд. Можем предложить из портфеля: " + ", ".join(alts[:5]))
        else:
            await m.answer("Похоже на чужой бренд, но подходящей альтернативы я не нашёл. Попробуйте уточнить запрос.")
